 ...
 * @author Dinuzhka
 */
public class EducationSearch implements ISearch {

    private int[] titleIds;

    public EducationSearch(int[] titleIds) {
    ...
    public List<Persons> getPersons(int title) {
        ...
        try {
            String hql = "FROM Persons as p join fetch p.educationLevels as jt  WHERE p.type='user' AND jt.idEducationLevel=:idEducationLevel";
            Query query = session.createQuery(hql);
            query.setParameter("idEducationLevel", title);
